...
func (s *Server) handleTunnel(w http.ResponseWriter, r *http.Request) {
	...
	// Compute proper Sec-WebSocket-Accept per RFC 6455 — fixed/wrong values
	// are trivially detected by Iran's DPI as fake WebSocket endpoints.
	...
	// Wrap in RFC 6455 WebSocket binary framing — makes post-handshake traffic
	// look like real WebSocket data instead of raw binary length-prefixed packets.
	wsConn := newWSFrameConn(conn, false) // server-side: no masking

	// Wrap with encryption
	ec, err := NewEncryptedConn(wsConn, s.PSK, s.Obfs, &s.Config.Stealth)
	if err != nil {
