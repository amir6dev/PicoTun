...
func applyBaseDefaults(c *Config) {
	...
	// DPI Stealth defaults — tuned for Iran's deep-inspection infrastructure.
	// Larger padding and jitter break size/timing fingerprints.
	if c.Stealth.MinPadding <= 0 {
		c.Stealth.MinPadding = 32
	}
	if c.Stealth.MaxPadding <= 0 {
		c.Stealth.MaxPadding = 256
	}
	if c.Stealth.KeepaliveJitter <= 0 {
		c.Stealth.KeepaliveJitter = 3
	}
	if c.Stealth.ConnJitterMS <= 0 {
		c.Stealth.ConnJitterMS = 800
	}
	if c.Stealth.MaxBurstSize <= 0 {
		c.Stealth.MaxBurstSize = 8192
	}
	if c.Stealth.FakeTrafficInterval <= 0 {
		c.Stealth.FakeTrafficInterval = 20
	}

	// Domain/UA rotation pools — use well-known domains with real WebSocket
	// usage to make the Host header believable. Avoid domains blocked in Iran.
	if len(c.Stealth.DomainPool) == 0 {
		c.Stealth.DomainPool = []string{
			// Google — widely accessible, has WS endpoints
			"accounts.google.com", "meet.google.com",
			"classroom.google.com", "docs.google.com",
			"mail.google.com", "drive.google.com",
			// Microsoft / Teams — widely used, known WS traffic
			"teams.microsoft.com", "login.microsoftonline.com",
			"outlook.live.com", "onedrive.live.com",
			// Cloudflare CDN — anycast IPs; no single IP range DPI can pin
			"cdnjs.cloudflare.com", "challenges.cloudflare.com",
			// Apple
			"gateway.icloud.com", "api.apple-cloudkit.com",
			// Amazon / AWS — broad CDN IP space
			"d1.awsstatic.com", "api.amazon.com",
			// Popular services with WS — believable upgrade context
