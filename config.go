...
func applyBaseDefaults(c *Config) {
	...
	if c.Advanced.TCPReadBuffer <= 0 {
		c.Advanced.TCPReadBuffer = 65536
	}
	if c.Advanced.TCPWriteBuffer <= 0 {
		c.Advanced.TCPWriteBuffer = 65536
	}
...
func migrateConfig(c *Config, path string) {
	...
	if c.ConfigVersion < 2 {
		if c.Smux.MaxRecv == 524288 {
			c.Smux.MaxRecv = 1048576
			log.Printf("[CONFIG]   smux.max_recv: 512KB → 1MB")
		}
		if c.Smux.MaxStream == 524288 {
			c.Smux.MaxStream = 1048576
			log.Printf("[CONFIG]   smux.max_stream: 512KB → 1MB")
		}
		...
		if c.Advanced.TCPReadBuffer == 32768 {
			c.Advanced.TCPReadBuffer = 65536
			log.Printf("[CONFIG]   tcp_read_buffer: 32KB → 64KB")
		}
		if c.Advanced.TCPWriteBuffer == 32768 {
			c.Advanced.TCPWriteBuffer = 65536
			log.Printf("[CONFIG]   tcp_write_buffer: 32KB → 64KB")
		}
