...
func (c *Client) connectAndServe(id int, path PathConfig) error {
	...
	// Wrap in RFC 6455 WebSocket binary framing — client MUST mask per spec.
	// Post-handshake traffic now looks like real WebSocket binary frames.
	conn = newWSFrameConn(conn, true)
